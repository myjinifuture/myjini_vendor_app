...
class UpdateProduct extends StatefulWidget {
  var ProductList,categoryid,subcategoryid;
  UpdateProduct(this.ProductList,this.categoryid,this.subcategoryid);
...
class _UpdateProductState extends State<UpdateProduct> {
  ...
  String Id,Price,Discount,Name,_url;
  ...
  _getData() async {
   // SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      Id = widget.ProductList["Id"].toString();
      txtName.text = widget.ProductList["Name"];
      txtPrice.text = widget.ProductList["Price"].toString();
      txtDiscount.text = widget.ProductList["Discount"].toString();
      _url = widget.ProductList["Image"];
    });
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        res.then((data) async {
          ...
          if (data != null && data.length > 0) {
            ...
            String currentData = widget.categoryid;
            print("pass data:$currentData");
            print("hello-> " + widget.categoryid);
            for (int i = 0; i < data.length; i++) {
              if (data[i].id == currentData) {
                setState(() {
                  _categoryClass = data[i];
                });
                getSubCategory(data[i].id);
                print("match");
                print("ff-> "+data[i].id);
              }
            }
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        res.then((data) async {
          ...
          if (data != null && data.length > 0) {
            ...
            if (_subCategoryClass == null) {
              SharedPreferences preferences =
              await SharedPreferences.getInstance();
              String currentData = widget.subcategoryid;
              print("hello-> " + widget.subcategoryid);
              print("c: ${currentData}");
              for (int i = 0; i < data.length; i++) {
                if (data[i].id == currentData) {
                  setState(() {
                    _subCategoryClass = data[i];
                  });
                  print("set");
                }
              }
  ...
  _updateProduct() async {
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        Services.UpdateProduct(data).then((data) async {
          pr.hide();
          if (data.Data != "0" && data.IsSuccess == true) {
            /*
            SharedPreferences prefs = await SharedPreferences.getInstance();
            await prefs.setString(cnst.Session.ProductImage, "${data.Data}");
*/
